...
class AlarmsController {
	public static void main(String args[]) {
		...
		boolean MotionState = false; // Motion: false == no motion, true ==
										// motion
		...
		if (em != null) {
			...
			MessageWindow mw = new MessageWindow("Alarm Controller Status",
					WinPosX, WinPosY);
			...
			while (!Done) {
				...
				for (int i = 0; i < qlen; i++) {
					...
					if (Msg.GetMessageId() == 7) {
						if (Msg.GetMessage().equalsIgnoreCase("Window")) // broken
																			// window
						{
							WindowState = true;
						...
						if (Msg.GetMessage().equalsIgnoreCase("Door")) // broken
																		// door
						{
							DoorState = false;
						...
						if (Msg.GetMessage().equalsIgnoreCase("Motion")) // motion
																			// detected
						{
							ArmedState = false;
