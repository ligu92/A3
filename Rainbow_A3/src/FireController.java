...
class FireController {
	public static void main(String args[]) {
		...
		boolean ArmedState = false; // Alarms armed: false == disarmed, true ==
									// armed
		boolean FireState = false; // Windows: false == no fire, true == fire
									// detected
		...
		if (em != null) {
			...
			MessageWindow mw = new MessageWindow("Fire Controller Status",
					WinPosX, WinPosY);
			...
			while (!Done) {
				...
				for (int i = 0; i < qlen; i++) {
					...
					if (Msg.GetMessageId() == 8) {
						if (Msg.GetMessage().equalsIgnoreCase("Fire")) // broken
																		// window
						{
							FireState = true;
							mw.WriteMessage("Received Fire Detection");
						...
						if (Msg.GetMessage().equalsIgnoreCase("FireOff")) // broken
																			// window
						{
							FireState = false;
							mw.WriteMessage("Recived Fire Off");
